// Code from https://github.com/nicksnyder/go-i18n/blob/main/v2/goi18n/extract_command.go
...
func i18nPackageName(file *ast.File) string {
	for _, i := range file.Imports {
		if i.Path.Kind == token.STRING && i.Path.Value == `"github.com/cuipeiyu/go-i18n"` {
			if i.Name == nil {
				return "i18n"
			}
			return i.Name.Name
